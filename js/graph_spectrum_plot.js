...
const BLUR_FILTER_PIXEL = 1,
  ...
  RPM_MARKER_TOP_MARGIN = 40, // keeps ctrl-hover RPM lines/labels clear of the toolbar combo boxes
  ZOOM_X_MAX = 5;
...
GraphSpectrumPlot.initialize = function (canvas, sysConfig, craftConfig) {
  this._canvasCtx = canvas.getContext("2d");
  this._sysConfig = sysConfig;
  this._craftConfig = craftConfig || null;
...
GraphSpectrumPlot.setRpmMousePosition = function (x, y) {
  this._rpmMousePosition.x = x;
  this._rpmMousePosition.y = y;
};
...
GraphSpectrumPlot._drawFrequencyGraph = function (canvasCtx) {
  ...
  this._drawAxisLabel(
    canvasCtx,
    this._fftData.fieldName + this._fftData.minMaxAvgText,
    WIDTH - 4,
    HEIGHT - 20,
    "right",
  );
...
GraphSpectrumPlot._drawNotCachedElements = function () {
  ...
  if (this._rpmMousePosition.x > 0 || this._rpmMousePosition.y > 0) {
    this._drawRpmMousePosition(
      canvasCtx,
      this._rpmMousePosition.x,
      this._rpmMousePosition.y,
      WIDTH,
      HEIGHT,
      15 * offset + MARGIN + RPM_MARKER_TOP_MARGIN,
    );
  }
...
GraphSpectrumPlot._drawRpmFrequency = function (
  canvasCtx,
  frequency,
  sampleRate,
  label,
...
GraphSpectrumPlot._drawLowpassDynFilter = function (
  ...
  // frequency2 line
  const offsetByType =
    this._spectrumType === SPECTRUM_TYPE.FREQ_VS_THROTTLE ? 0 : OFFSET;
...
GraphSpectrumPlot._drawMousePosition = function (
  ...
  if (
    ...
    const linewidth = 2;
    const linecolour = "rgba(30,144,255,0.85)"; //"rgba(0,0,255,0.66)";
    ...
    if (mouseFrequency >= 0 && mouseFrequency <= sampleRate) {
      this._drawRpmFrequency(
        ...
        mouseFrequency,
        sampleRate,
        ` (${Math.round(mouseFrequency * 60)}rpm)`,
        WIDTH,
        HEIGHT,
        OFFSET,
        linecolour,
...
/*
 * Ctrl+hover marker: shows main rotor RPM (blue) at the hovered frequency, plus tail rotor RPM
 * (red) and motor RPM (yellow) derived from it via the gear ratios loaded through the Expert
 * "Load Config" feature (js/craft_config.js). The blackbox log itself has no gear ratio data.
 */
GraphSpectrumPlot._drawRpmMousePosition = function (
  canvasCtx,
  mouseX,
  mouseY,
  WIDTH,
  ...
  const sampleRate = this._fftData.blackBoxRate / this._zoomX;
  const marginLeft = this._getActualMarginLeft();
  const mouseFrequency = ((mouseX - marginLeft) / WIDTH) * (sampleRate / 2);
  const width = 2;
  ...
  const mainRotorRpm = mouseFrequency * 60;
  this._drawVerticalMarkerLine(
    canvasCtx,
    mouseFrequency,
    sampleRate / 2,
    `Main ${Math.round(mainRotorRpm)}rpm`,
    WIDTH,
    HEIGHT,
    OFFSET,
  ...
  const gearRatios = this._getGearRatios();

  // Tail rotor RPM, derived from main rotor RPM via tail_rotor_gear_ratio
  if (gearRatios && gearRatios.tail) {
    const tailRotorRpm =
      (mainRotorRpm * gearRatios.tail[1]) / gearRatios.tail[0];
    const tailFrequency = tailRotorRpm / 60;
    if (tailFrequency >= 0 && tailFrequency <= sampleRate) {
      this._drawVerticalMarkerLine(
        canvasCtx,
        tailFrequency,
        sampleRate / 2,
        `Tail ${Math.round(tailRotorRpm)}rpm`,
        WIDTH,
        HEIGHT,
        OFFSET + 15,
        "rgba(255,51,51,0.85)",
  ...
  // Motor RPM, derived from main rotor RPM via main_rotor_gear_ratio (skipped when direct drive, i.e. 1:1)
  if (
    gearRatios &&
    gearRatios.main &&
    gearRatios.main[0] !== gearRatios.main[1]
  ) {
    const motorRpm = (mainRotorRpm * gearRatios.main[1]) / gearRatios.main[0];
    const motorFrequency = motorRpm / 60;
    if (motorFrequency >= 0 && motorFrequency <= sampleRate) {
      this._drawVerticalMarkerLine(
        canvasCtx,
        motorFrequency,
        sampleRate / 2,
        `Motor ${Math.round(motorRpm)}rpm`,
        WIDTH,
        HEIGHT,
        OFFSET + 30,
        "rgba(255,214,0,0.85)",
...
GraphSpectrumPlot._getGearRatios = function () {
  if (!this._craftConfig || !this._craftConfig.getSettings) {
    return null;
  }

  const settings = this._craftConfig.getSettings();
  return {
    main: this._parseGearRatio(settings["main_rotor_gear_ratio"]),
    tail: this._parseGearRatio(settings["tail_rotor_gear_ratio"]),
  };
};

// Rotorflight CLI stores gear ratios as "numerator,denominator" (e.g. "16,155")
GraphSpectrumPlot._parseGearRatio = function (rawValue) {
  if (!rawValue) {
    return null;
  }
