...
function FlightLogGrapher(flightLog, graphConfig, canvas, stickCanvas, craftWrapper, analyserCanvas, stepResponseCanvas, options) {
    ...
    var
        ...
        defaultOptions = {
            ...
            drawStepResponse:false,         // add a step response option
        ...
        sticks = null,
            ...
            stepResponse = null, /* define a new step response panel */
    ...
    this.render = function(windowCenterTimeMicros) {
        ...
        if (chunks.length) {
            ...
            // Draw Step Response (always shows roll/pitch/yaw, independent of selected field)
            if (options.drawStepResponse) {
                try {
                    stepResponse.plot();
    ...
    this.setInTime = function(time) {
        ...
        stepResponse.setInTime(inTime);

        if (outTime <= inTime) {
            outTime = false;
            analyser.setOutTime(outTime);
            stepResponse.setOutTime(outTime);
        }
    };
    ...
    this.setStepResponseFullscreen = function(state) {
      stepResponse.setFullscreen( state );
    };
