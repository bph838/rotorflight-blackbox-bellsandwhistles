...
StepResponsePlot._drawGraph = function(canvasCtx) {
    ...
    const HEIGHT = canvasCtx.canvas.height - STEP_RESPONSE_MARGIN_BOTTOM - STEP_RESPONSE_MARGIN;
...
StepResponsePlot._drawGrid = function(canvasCtx, WIDTH, HEIGHT) {
    ...
    for (let i = 0; i <= TICKS; i++) {
        ...
        canvasCtx.strokeStyle = (Math.abs(value - 1.0) < 1e-6) ? "rgba(255,255,255,0.6)" : "rgba(255,255,255,0.2)";
    ...
    for (let i = 0; i <= TICKS; i++) {
        ...
        this._drawLabel(canvasCtx, Math.round(value * 1000) + 'ms', x, HEIGHT + 4, 'center', 'top');
    }
};
...
StepResponsePlot._drawMousePosition = function(canvasCtx) {
    ...
    this._drawLabel(canvasCtx, Math.round(timeSec * 1000) + 'ms', labelX, 2, labelAlign, 'top', "rgba(0,255,0,0.9)");
    ...
    for (const axis in STEP_RESPONSE_AXIS_LABELS) {
        ...
        this._drawLabel(canvasCtx, value.toFixed(2), labelX, 2 + lineIndex * 12, labelAlign, 'top', STEP_RESPONSE_AXIS_COLORS[axis]);
        lineIndex++;
    }
...
StepResponsePlot._interpolateResponseAt = function(axisData, timeSec) {
    ...
    const time = axisData.time, response = axisData.response;
    ...
    const index = Math.min(response.length - 2, Math.max(0, Math.floor(fraction * (response.length - 1))));
